...
def _engine_note(engine: str) -> str:
    ...
    if engine == ENGINE_SOS:
        return "扫描器已执行 RULES5：E_index 或 E_sos（禁止双均线否决），最多 3 条主题当天可点火。可买 000/001/002/003/600/601/603/605，排除 300/688/北证/ST。T-1 收盘≥MA20×0.97 入池，T 日池内判 A/B。涨停视为 BUY_A 量能满足。回测不扫 5000。试仓不是成交指令。"
